<style>
    ...
    body {
        font-family: 'Roboto', sans-serif; /* Mengatur font halaman menjadi Roboto */
        position: relative; /* Posisi relatif agar elemen-elemen dapat diposisikan berdasarkan elemen ini */
    }
</style>
...
<?php if (isset($_SESSION['success'])): ?>
    ...
    <div id="success-notification" class="bg-green-500 text-white p-2 rounded shadow-lg absolute top-4 right-4 text-sm z-50">
        <?= $_SESSION['success']; ?>
        <?php unset($_SESSION['success']); ?> <!-- Menghapus pesan success setelah ditampilkan -->
    </div>
<?php endif; ?>
...
<body>
    ...
    <script>
        ...
        window.onload = function() {
            ...
            if (notification) {
                setTimeout(() => {
                    notification.classList.add('hidden'); // Menyembunyikan notifikasi setelah 2 detik
                    window.location.href = "<?= BASEURL; ?>/home/contact"; // Mengarahkan kembali ke halaman kontak
                }, 2000);
            }
        };
    </script>
