...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (notification) {
            setTimeout(() => {
                notification.classList.add('opacity-0'); // Tambahkan kelas untuk memudarkan
                setTimeout(() => notification.remove(), 500); // Hapus elemen setelah transisi
            }, 3000);
        }
    });
</script>
...
<style>
    #notification {
        ...
        font-size: 0.875rem; /* Menggunakan ukuran font lebih kecil */
    }
    .opacity-0 {
        opacity: 0;
    }
...
<style>
    /* Menambahkan transisi agar efek memudar lebih halus */
    #notification {
        opacity: 1;
        transition: opacity 0.5s ease;
    }
...
<div class="container mx-auto py-12 px-4">
    ...
    <div class="flex flex-col md:flex-row justify-center mt-12">
        <div class="bg-black text-white p-8 md:p-10 rounded-lg w-full md:w-2/5 lg:w-2/5">
            ...
            <p class="mb-6 text-xs md:text-sm">If you have any questions or concerns, you can contact us by filling out the contact form, calling us, visiting our office, finding us on other social networks, or you can send us a personal email at:</p>
            <div class="flex items-center mb-6 text-xs md:text-sm">
        ...
        <div class="w-full md:w-2/5 lg:w-2/5 md:ml-8 mt-8 md:mt-0">
            <form action="<?= BASEURL; ?>/home/btnContact" method="POST" class="space-y-4 md:space-y-6">
                <div>
                    <label for="name" class="block text-gray-700 text-xs md:text-sm">Name</label>
                    <input type="text" id="name" name="name" class="w-full border border-gray-300 p-3 rounded text-xs md:text-sm" required>
                </div>
                <div>
                    <label for="email" class="block text-gray-700 text-xs md:text-sm">Email</label>
                    <input type="email" id="email" name="email" class="w-full border border-gray-300 p-3 rounded text-xs md:text-sm" required>
                </div>
                <div>
                    <label for="phone" class="block text-gray-700 text-xs md:text-sm">Phone Number</label>
                    <input type="text" id="phone" name="phone" class="w-full border border-gray-300 p-3 rounded text-xs md:text-sm">
                </div>
                <div>
                    <label for="message" class="block text-gray-700 text-xs md:text-sm">Message</label>
                    <textarea id="message" name="message" class="w-full border border-gray-300 p-3 rounded h-32 md:h-40 text-xs md:text-sm" required></textarea>
                </div>
                <button type="submit" class="bg-black text-white py-3 px-6 rounded w-full text-xs md:text-sm">SEND</button>
            </form>
        </div>
    </div>
    <div class="mt-12">
